document.addEventListener('DOMContentLoaded', () => {
    lucide.createIcons();
    initApp();
    setupVideoScrollSync();
});
// Sync video playback with scroll position
function setupVideoScrollSync() {
    const video = document.getElementById('showcase-video');
    const videoLanding = document.getElementById('video-landing');
    if (!video || !videoLanding) return;
    ...
    video.addEventListener('ended', () => {
        ...
        if (blendOverlay) blendOverlay.style.opacity = '1';
        // Enable scrolling after fade, but keep landing section visible
        setTimeout(() => {
            document.body.style.overflow = '';
            // Do NOT hide videoLanding, keep it in place
            // Optionally, show a scroll indicator or prompt
